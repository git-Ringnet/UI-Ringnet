...
class mod_quiz_renderer extends plugin_renderer_base
{
    ...
    public function review_form($page, $showall, $displayoptions, $content, $attemptobj)
    {
        ...
        $output = '';
        $output .= html_writer::start_tag('form', array('action' => $attemptobj->review_url(
            null,
            $page,
            $showall
    ...
    protected function render_quiz_nav_question_button(quiz_nav_question_button $button)
    {
        ...
        $tagcontents = 
            ...
            html_writer::tag('span', $button->statestring, array('class' => 'state')) .
            html_writer::tag('span', $svg, array('class' => 'mysvg'));;
        // $tagcontents = html_writer::tag('span', '', array('class' => 'thispageholder')) .
        //     get_string($qnostring, 'quiz', $a);
        // $tagcontents = html_writer::div();
        $tagattributes = array(
    ...
    public function attempt_page(
        ...
        $output .= html_writer::div($this->countdown_timer($attemptobj, time()), 'py-2');
        $output .= $this->footer();
        return $output;
    }
    ...
    protected function attempt_navigation_buttons($page, $lastpage, $navmethod = 'free')
    {
        ...
        $output .= html_writer::start_tag('div', array('class' => 'submitbtns-pre-next pt-3'));
        if ($page > 0 && $navmethod == 'free') {
            $output .= html_writer::empty_tag('input', array(
                'type' => 'submit', 'name' => 'previous',
                'value' => '< '.get_string('navigateprevious', 'quiz'), 'class' => 'mod_quiz-prev-nav btn-prev mr-3',
    ...
    {
        $attributes = array(
            'type' => 'submit',  'name' => 'redoslot' . $slot,
    ...
    public function summary_table($attemptobj, $displayoptions)
    {
        ...
        foreach ($slots as $slot) {
            ...
            if ($attemptobj->is_question_flagged($slot)) {
                ...
                $flag = html_writer::empty_tag('img', array(
